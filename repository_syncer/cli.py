...
def _fetch_gitlab_known_hosts() -> List[str]:
...
def _fetch_github_known_hosts() -> List[str]:
    logger.info("Downloading Github known_hosts file")
    r = requests.get(
    ...
    r.raise_for_status()
    data = r.json()
    return [f"github.com {ssh_key}" for ssh_key in data["ssh_keys"]]
...
def _setup_environment():
    ...
    if not manage_environment:
        logger.info("Skipping ssh known_hosts file due to manage_environment")
    else:
        ssh_known_hosts = _fetch_gitlab_known_hosts() + _fetch_github_known_hosts()
    ...
    if not encoded_ssh_key:
        raise RuntimeError("Missing ssh-key, ensure GITLAB_SSH_KEY is set")
